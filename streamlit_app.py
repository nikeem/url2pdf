...
def ensure_browser_installed() -> None:
    """
    Проверяет и устанавливает браузер Playwright.
    Кэшируется между запусками - выполняется только один раз за сессию.
    """
    try:
        ...
        try:
            ...
            raise RuntimeError("Не удалось установить браузер Chromium")
...
def validate_url(url: str) -> tuple[bool, str]:
    ...
    if parsed.hostname is None or parsed.hostname in BLOCKED_HOSTS:
        hostname = parsed.hostname or "None"
        return False, f"Заблокированный хост: '{hostname}'. Доступ к внутренним ресурсам запрещён"
...
def generate_pdf(
    ...
    """
    # Убеждаемся, что браузер установлен (с кэшированием)
    try:
...
# Кнопка генерации (отключена, если URL пустой)
is_url_empty = not url_input or not url_input.strip()
generate_btn = st.button(
    "🚀 Создать PDF",
    type="primary",
    use_container_width=True,
    disabled=is_url_empty
)

# Подсказка, если URL пуст
if is_url_empty:
    st.caption("💡 Введите URL выше, чтобы активировать кнопку")
